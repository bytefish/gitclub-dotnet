...
set PGSQL_EXECUTABLE="C:\Program Files\PostgreSQL\16\bin\psql.exe"
set CREATEDB_EXECUTABLE="C:\Program Files\PostgreSQL\16\bin\createdb.exe"
...
1>%STDOUT% 2>%STDERR% (
	...
	%PGSQL_EXECUTABLE% -h %HostName% -p %PortNumber% -d %DatabaseName% -U %UserName% < gitclub.sql -L %LOGFILE%
)
